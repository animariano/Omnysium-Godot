...
func actualizar_ui():
    ...
    $Background/HeroePanel/VBoxContainer/HeroeStatsLabel.text = "⚔ %d     🪙 %d" % [h.ataque, h.oro]	
    ...
    $Background/EnemigoPanel/VBoxContainer/EnemigoStatsLabel.text = "⚔ %d" % ataque_enemigo
...
func chequear_muerte_enemigo():
    if salud_enemigo <= 0:
        log_combate("[b]¡Venciste al " + GameManager.enemigo_actual().nombre + "! Ganaste 3 🪙.[/b]")
