...
func _on_especial():
	...
	match h.clase:
		"Paladin":
			GameManager.heroe.salud = min(h.salud + 4, h.salud_maxima)
...
func configurar_botones_accion():
	var h = GameManager.heroe
	var btn_atacar = self.btn_atacar 
	var btn_especial  = self.btn_especial
	var btn_definitiva = self.btn_definitiva
