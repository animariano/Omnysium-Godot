...
var personajes_pool = [
    ...
    {
    "nombre": "Asesino", "clase": "Asesino",
    "salud": 6, "ataque": 3,
    "habilidad_especial": "Veneno (aplica veneno 3 turnos, 1 daño por turno)",
    "habilidad_definitiva": "Ejecución (A enemigos con 50% o menos de salud)",
    "cooldown_definitiva": 2
    },
    {
        "nombre": "Caballero Carmesí", "clase": "Caballero_carmesi",
        "salud": 10, "ataque": 2,
        "habilidad_especial": "Ritual de poder (-2 de salud, +1 de ataque)",
        "habilidad_definitiva": "Pacto de sangre (El proximo ataque recibido, te cura, y devuelve el daño)",
...
func aplicar_efecto(objetivo: String, id: String, duracion: int, valor: int = 0):
    ...
    for e in lista:
        if id == "veneno":
                e.valor += valor  # ← apilar daño
                e.duracion = max(e.duracion, duracion)  # ← tomar la duración mayor
        else:
                e.duracion = duracion
                e.valor = valor
        return
