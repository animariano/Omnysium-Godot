...
func _on_trampa_dado():
	...
	if resultado <= 4:
		...
		log_evento("[color=red]No lograste esquivar la trampa. Salud: %d[/color]" % GameManager.heroe.salud)
...
func cargar_fuente():
	...
	log_evento("Tu salud se duplica. Salud: %d" % GameManager.heroe.salud)
	terminar_evento()
...
func _on_cueva_oscuro_dado():
	limpiar_opciones()
	await get_tree().process_frame  # ← agregá esta línea
	var resultado = randi() % 6 + 1
	#mostrar_dado(resultado)
	await animar_dado(resultado)
	if resultado <= 3:
		log_evento("[color=green]Esquivaste la roca! Salís ileso.[/color]")
		log_evento("En el fondo encontrás un cuchillo extraño...")
		await get_tree().process_frame  # ← agregá esta línea
		agregar_boton("Tomarlo (+2 salud, +2 ataque)", _on_cueva_tomar_cuchillo)
